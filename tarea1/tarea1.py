...
def learnPredictor(trainExamples: List[Tuple[T, int]],
    ...
    for epoch in range(numEpochs):  
        for x,y in trainExamples: 
            ...
            if y*dotProduct(phi,weights) <= 0: 
                increment(weights, eta*y, phi) 
    validationExamples = evaluatePredictor(validationExamples, lambda x: (1 if dotProduct(featureExtractor(x), weights) >= 0 else -1)) 
    trainExamples = evaluatePredictor(trainExamples, lambda x: (1 if dotProduct(featureExtractor(x), weights) >= 0 else -1)) 
    # FIN
    return weights
    # FIN
    return weights
